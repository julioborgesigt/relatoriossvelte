<script lang="ts">
    ...
    let etapa = $state<"matricula" | "token">(
        (data.etapa as "matricula" | "token") ?? "matricula",
    );
    let servidor = $state<{
        nome: string;
        matricula: string;
...
<div class="min-h-screen bg-[#0a192f] flex items-center justify-center p-4">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8">
            <div
                class="inline-block bg-[#c5a059] text-[#0a192f] font-black text-xs px-3 py-1 rounded mb-4 uppercase tracking-widest"
            >
                Polícia Civil do Estado do Ceará
            </div>
            <h1
                class="text-3xl font-black text-[#c5a059] tracking-tight uppercase"
            >
                Sistema de Plantões
            </h1>
            <p class="text-slate-400 text-sm mt-1 uppercase tracking-wider">
                DPI SUL — Acesso Seguro
            </p>
        </div>

        <div
            class="bg-white/5 border border-[#c5a059]/30 rounded-xl p-8 shadow-2xl"
        >
            {#if etapa === "matricula"}
                <!-- Etapa 1: Inserir matrícula -->
                <h2
                    class="text-[#c5a059] font-bold text-sm uppercase tracking-widest mb-6"
                >
                    Identificação
                </h2>
                ...
                >
                    <div class="mb-5">
                        <label
                            for="matricula"
                            class="block text-slate-300 text-xs font-bold uppercase mb-2"
                        >
                            Número de Matrícula
                        </label>
                        <input
                    ...
                    {#if form?.erro}
                        <div
                            class="mb-4 bg-red-900/40 border border-red-500/50 text-red-300 text-sm p-3 rounded-lg"
                        >
                            {form.erro}
                        </div>
                    {/if}
            ...
            {:else if etapa === "token" && servidor}
                ...
                <div class="mb-6">
                    <div
                        class="bg-[#c5a059]/10 border border-[#c5a059]/30 rounded-lg p-4 mb-4"
                    >
                        <p
                            class="text-[#c5a059] text-xs font-bold uppercase mb-1"
                        >
                    ...
                    <p class="text-slate-400 text-sm text-center">
                        Código enviado para: <span class="text-white font-mono"
                            >{servidor.emailMascarado}</span
                        >
                    </p>

                    {#if tokenDev}
                        <div
                            class="mt-3 bg-yellow-900/40 border border-yellow-500/50 rounded-lg p-3 text-center"
                        >
                            <p
                                class="text-yellow-400 text-xs font-bold uppercase mb-1"
                            >
                                Modo Dev — Token:
                            </p>
                            <p
                                class="text-yellow-300 font-mono text-2xl font-black tracking-[0.5em]"
                            >
                                {tokenDev}
                            </p>
                        </div>
